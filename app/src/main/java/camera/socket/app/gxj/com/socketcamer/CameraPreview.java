...
public class CameraPreview extends SurfaceView implements
	...
	public CameraPreview(Context context, Camera camera) {
		...
		Log.i(TAG, "preview size = " + mPreviewSize.width + ", "
				+ mPreviewSize.height);
	...
	private void saveYUV(byte[] byteArray) {
		...
		YuvImage im = new YuvImage(byteArray, ImageFormat.NV21,
		...
		try {
			FileOutputStream output = new FileOutputStream(
					Environment.getExternalStorageDirectory() + "/yuv.jpg");
	...
	private void saveRAW(byte[] byteArray) {
		try {
			FileOutputStream file = new FileOutputStream(new File(
					Environment.getExternalStorageDirectory() + "/test.yuv"));
			try {
